  ...
  return (
    <div className='mt-5 '>
      ...
      <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-5 '>
        {businessList && businessList.length>0?businessList.map((business,index)=>(
            <Link href={'/details/'+business.id}
             key={index} className='shadow-md rounded-lg hover:shadow-lg cursor-pointer
            hover:shadow-primary hover:scale-105 transition-all ease-in-out' >
                <Image src={business.images[0].url} alt={business.name}
                width={500}
                height={200} className='h-[150px] md:h-[200px] 
